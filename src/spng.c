        ...
        else /* Arbitrary-length chunk */
        {
                     ...
                     !memcmp(chunk.type, type_itxt, 4))
            {
                ...
                else if(!memcmp(chunk.type, type_itxt, 4))
                {
                    ...
                    const unsigned char *zlib_stream2 = memchr(data + translated_keyword_offset, 0, peek_bytes - translated_keyword_offset);
                    if(zlib_stream2 == NULL) return SPNG_EITXT;
                    if(zlib_stream2 == peek_end) return SPNG_EITXT;

                    text_offset = zlib_stream2 - data + 1;
