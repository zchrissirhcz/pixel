...
static void fc_glfw_show_image(const char* title, const Image* im);
...
static unsigned int create_texture(const Image* im)
{
    int x, y;
    GLuint texture;
...
void fc_glfw_show_image(const char* title, const Image* im)
{
    int x, y, width;
    srand(time(0));
    ...
    FcGlfwWindow* win = fc_glfw_get_window(title, im->width, im->height);
