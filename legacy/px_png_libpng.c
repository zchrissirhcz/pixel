...
void px_read_png(const char* filename, unsigned char** data, size_t* w, size_t* h)
{
    ...
    *data = (unsigned char*)malloc(*h * png_get_rowbytes(png_ptr, info_ptr) + sizeof(png_bytep) * 16);
    for (unsigned int ii = 0; ii < *h; ii++)
    {
        png_read_row(png_ptr, *data + ii * png_get_rowbytes(png_ptr, info_ptr), NULL);
    }
...
void px_write_png(const char* filename, unsigned char* data, size_t w, size_t h ,size_t bitdepth)
{
    ...
    png_set_IHDR(png_ptr, info_ptr, w, h, bitdepth, PNG_COLOR_TYPE_GRAY,
                 // PNG_COLOR_TYPE_RGBA,
