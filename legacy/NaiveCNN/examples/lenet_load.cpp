...
static void load_lenet_C1_data(Lenet* net, FILE* fin)
{
    ...
    load_weight4d(net->C1->kernel->data, tensor_dim, fin);
    load_bias(net->C1->bias->data, net->C1->out_channels, fin);
}
...
static void load_lenet_C3_data(Lenet* net, FILE* fin)
{
    ...
    px_tensor_dim_t tensor_dim = px_create_tensor_dim(batch, channel, height, width);
    load_weight4d(net->C3->kernel->data, tensor_dim, fin);

    load_bias(net->C3->bias->data, net->C3->out_channels, fin);
}
...
static void load_lenet_O5_data(Lenet* net, FILE* fin)
{
    ...
    load_weight2d(net->O5->weight->data, size, fin);

    load_bias(net->O5->bias->data, net->O5->output_num, fin);
}
