...
void bright_contrast(unsigned char* src_rgb, size_t height, size_t width, int bright, int contrast, unsigned char* dst_rgb)
{
    ...
    for (int i=0; i<256; i++) {
        int temp = contrast > 0 ? pixel_clip(i+bright, 0, 255) : i;
        if (contrast>0) {
            temp = pixel_clip(i+bright, 0, 255);
            temp = pixel_clip(average + (temp-average)*(1.0f/(1.0f - contrast/100.f)), 0, 255);
        } else {
            temp = i;
            temp = pixel_clip(average +(temp-average)*(1.0f + contrast/100.0f), 0, 255);
        }
        bc_map[i] = temp;
    }
    src_ptr = src_rgb;
