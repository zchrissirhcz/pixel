...
static void matrix_column_max_and_idx_test()
{
    ...
    size_t* max_indicies_neighborline = (size_t*)malloc(buf_size);
    memset(max_indicies_neighborline, 0, buf_size);

    size_t* max_indicies_neighborline_asimd = (size_t*)malloc(buf_size);
    memset(max_indicies_neighborline_asimd, 0, buf_size);
    ...
    dst_buf = result_neighborline.data;
    t_start = pixel_get_current_time();
    matrix_column_max_and_idx_u8_neighborline(src_buf, height, width, dst_buf, max_indicies_neighborline);
    t_cost = pixel_get_current_time() - t_start;
    PIXEL_LOGD("matrix column max and idx u8, neighborline, time cost %4lf ms\n", t_cost);

    // neighbor line asimd
    src_buf = gray.data;
    dst_buf = result_neighborline_asimd.data;
    t_start = pixel_get_current_time();
    matrix_column_max_and_idx_u8_neighborline_asimd(src_buf, height, width, dst_buf, max_indicies_neighborline_asimd);
    t_cost = pixel_get_current_time() - t_start;
    PIXEL_LOGD("matrix column max and idx u8, neighborline asimd, time cost %4lf ms\n", t_cost);
    ...
    for (size_t i = 0; i < width; i++) {
        ...
        if ( (max_indicies_naive[i] != max_indicies_opencv[i])
              || (max_indicies_naive[i] != max_indicies_neighborline[i])
             //(max_indicies_naive[i] != max_indicies_neighborline_asimd[i])
            ) {
            mis_idx_count++;
        }
    }
    ...
    free(max_indicies_neighborline);
    free(max_indicies_neighborline_asimd);
