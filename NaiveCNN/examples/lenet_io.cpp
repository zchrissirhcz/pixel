...
static void save_conv_layer_data(ConvLayer* conv_layer, px_size_t output_size, FILE* fout)
{
    save_conv_weight_data(conv_layer, fout);
    save_bias_data(conv_layer->biasData, conv_layer->out_channels, fout);

    px_cube_dim_t cube_dim = px_create_cube_dim(conv_layer->out_channels, output_size.height, output_size.width);
...
{
    px_size_t output_size = px_create_size(net->S2->in_height, net->S2->in_width);
    save_conv_layer_data(net->C1, output_size, fout);
}

static void save_lenet_C3_layer_data(Lenet* net, FILE* fout)
{
    px_size_t output_size = px_create_size(net->S4->in_height, net->S4->in_width);
    save_conv_layer_data(net->C3, output_size, fout);
}

static void save_pooling_layer_data(PoolingLayer* pool_layer, px_size_t output_size, FILE* fout)
{
    px_cube_dim_t cube_dim = px_create_cube_dim(pool_layer->out_channels, output_size.height, output_size.width);
...
{
    px_size_t output_size = px_create_size(net->C3->in_height, net->C3->in_width);
    save_pooling_layer_data(net->S2, output_size, fout);
}

static void save_lenet_S4_layer_data(Lenet* net, FILE* fout)
{
    px_size_t output_size = px_create_size(net->S4->in_height / net->S4->map_size, net->S4->in_width / net->S4->map_size);
    save_pooling_layer_data(net->S4, output_size, fout);
}

static void save_lenet_O5_layer_data(Lenet* net, FILE* fout)
{
    px_size_t size = px_create_size(net->O5->outputNum, net->O5->inputNum);
    save_blob2d_to_file(net->O5->wData, size, fout);
    save_bias_data(net->O5->biasData, net->O5->outputNum, fout);
